...
public class SystemUtils {
    ...
    public static boolean isNeedRequestPermissions(Activity activity, @NonNull String permission) {
        boolean bool = false;
        //先判断版本是否大于23
        if (SystemUtils.canMakeSmores()) {
            //大于23，再判断是否已经申请
            if (SystemUtils.checkSelfPermission(activity, permission)) {
                //已申请
            } else {
                bool = true;
            }
    ...
    public static void savePushID(Context context,String JPushId){
        SharedPreferenceUtil.getInstance(context).saveString("JPushId", JPushId);
    }
    public static String getPushID(Context context){
        return SharedPreferenceUtil.getInstance(context).getString("JPushId", "");
    }
}
